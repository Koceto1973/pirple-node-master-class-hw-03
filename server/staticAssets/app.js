...
// Each form on the page gets 'submit' listener
  ...
  if(document.querySelector("form")){ // if there are forms on the page
    ...
    for(var i = 0; i < allForms.length; i++){ // and for each form
        ...
        for(var i = 0; i < elements.length; i++){
          if(elements[i].type !== 'submit'){ // do not count the submit button
            ...
            if(nameOfElement == '_method'){ 
              ...
              // adding the element to the payload
              if(classOfElement.indexOf('multiselect') > -1){ // case class "multiselect" add value(s) as array elements
                if(elementIsChecked){
        ...
        app.client.request(undefined,path,method,queryStringObject,payload,function(statusCode,responsePayload){
          ...
          if(statusCode !== 200){
            if(statusCode == 403){ // forbidden, log the user out
              app.logUserOut();
            } else {
